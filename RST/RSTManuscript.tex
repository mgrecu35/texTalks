...
89-GHz, while the others are centered on the 183.31, and 325.15 GHz water vapor absorption lines. Details regarding
the radiometer and the active instruments are
provided in Table \ref{t_sensors}.  Errors in the radiometer observations are modeled assuming a 
Noise-Equivalent-Delta-T (NEDT) of 1K, which is a readily achievable level for modern satellite radiometers \citep{draper2015}.
...
observations exhibits a multimodal narrow distribution. Moreover, there correlation between the retrieved 
and the true IWC gravity center is rather low.  This is another indication that, while the total amount of ice may be 
reasonably estimated from radiometer-only observations, its vertical distribution can not be 
accurately determined from radiometer-only observations.
